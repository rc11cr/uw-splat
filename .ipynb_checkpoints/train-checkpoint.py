...
from underwater.models import (
    ...
    BackscatterNet,
    BackscatterNetV2,
    BackscatterMLP,   # MLP 版 backscatter
    AttenuateMLP,     # MLP 版 attenuation
)
...
def training(model_params, opt_params, pipe_params, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    # 这里直接使用 MLP 版 Backscatter 和 Attenuation
    # ------------------------------------------------------------------
    if opt_params.do_seathru:
        # ✅ 固定使用 residual 版本，保证 checkpoint 里有 J_prime 等参数
    ...
    while iteration < opt_params.iterations + 1:
        ...
        # Freeze / unfreeze Gaussian parameters
        ...
        if opt_params.learn_background:
            if opt_params.bg_from_bs and opt_params.do_seathru and iteration > opt_params.seathru_from_iter:
                # 不再叠加 learned_bg，完全交给 backscatter
            ...
            if opt_params.do_seathru and iteration > opt_params.seathru_from_iter:
                if opt_params.alpha_binf_uw or opt_params.alpha_binf_render or opt_params.alpha_bg_opacities:
                    ...
                    if opt_params.alpha_binf_uw:
                        alpha_bg_loss += alpha_bg_criterion(
                            underwater_image.squeeze().detach(), torch.sigmoid(bs_model.B_inf.detach()), image_alpha
                        )
                    ...
                    if opt_params.alpha_binf_render:
                        alpha_bg_loss += alpha_bg_criterion(
                            rendered_image.detach(), torch.sigmoid(bs_model.B_inf.detach()), image_alpha
                        )
                    if opt_params.alpha_bg_opacities:
                        alpha_bg_loss += alpha_bg_criterion(
                            rgb_colors.detach(), torch.sigmoid(bs_model.B_inf.detach()).squeeze(), gaussians.get_opacity.squeeze()
                        )
                    ...
                    if opt_params.turn_off_bg_loss:
                        ...
                        alpha_bg_loss = alpha_bg_criterion(
                            underwater_image.squeeze().detach(), torch.sigmoid(learned_bg.detach()), image_alpha
                        )
            if not opt_params.turn_off_bg_loss:
        ...
        if opt_params.use_depth_smooth_loss:
            ...
            loss += opt_params.depth_smooth_lambda * depth_smooth_loss
            ...
            if iteration < opt_params.densify_until_iter:
                if iteration < opt_params.freeze_gs_from_iter:
                    ...
                    if iteration % opt_params.densification_interval == 0:
                        ...
                        if iteration > opt_params.densify_from_iter:
                            gaussians.densify_and_prune(
                                opt_params.densify_grad_threshold, 0.005, scene.cameras_extent, size_threshold
                            )
                    ...
                    if iteration % opt_params.densification_interval == 0:
                        ...
                        if iteration > opt_params.densify_from_iter:
                            gaussians.densify_and_prune(
                                opt_params.scale_grad_threshold * opt_params.densify_grad_threshold,
                                0.005,
    ...
    if not skip_eval_train:
        metrics_dirs.append(train_image_dir)
        with torch.no_grad():
            render_set(
...
def training_report(
    ...
    rgb_01_loss,
    rgb_01_criterion,
    depth_norm_value=1.0,
    norm_depth_max=False,
    depth_l1_loss=None,
    use_gt_depth=False,
    do_seathru=False,
    seathru_from_iter=9999999999,
    bg_from_bs=False,
    bs_loss=None,
    at_loss=None,
    bs_model=None,
    ...
    do_z_score=False,
    filter_depth=False,
    disable_attenuation=False,
    dcp_loss=None,
    dcp_criterion=None,
    depth_smooth_loss=None,
    depth_smooth_criterion=None,
    alpha_smooth_loss=None,
    gw_loss=None,
    gw_criterion=None,
    rgb_sv_loss=None,
    rgb_sv_criterion=None,
    rgb_sat_loss=None,
    rgb_sat_criterion=None,
    alpha_bg_loss=None,
    alpha_bg_criterion=None,
    ...
    if tb_writer:
        ...
        if do_learn_bg:
            tb_writer.add_scalar("background/r", torch.sigmoid(learned_bg)[0].item(), global_step=iteration)
            tb_writer.add_scalar("background/g", torch.sigmoid(learned_bg)[1].item(), global_step=iteration)
            tb_writer.add_scalar("background/b", torch.sigmoid(learned_bg)[2].item(), global_step=iteration)
            if iteration % 100 == 0:
                tb_writer.add_images(
                    "background/image", torch.sigmoid(learned_bg).reshape(1, 3, 1, 1), global_step=iteration
                )
    ...
    if iteration in testing_iterations:
        ...
        validation_configs = (
            {"name": "eval_test", "cameras": scene.getTestCameras()},
            {
                "name": "eval_train",
                "cameras": [
                    scene.getTrainCameras()[idx % len(scene.getTrainCameras())] for idx in range(5, 51, 5)
                ],
            },
        )
        ...
        for config in validation_configs:
            if config["cameras"] and len(config["cameras"]) > 0:
                ...
                for idx, viewpoint in enumerate(config["cameras"]):
                    ...
                    if filter_depth:
                        ...
                        if torch.any(torch.logical_or(torch.isnan(depth_image), torch.isinf(depth_image))):
                            valid_depth_vals = depth_image[
                                torch.logical_not(
                                    torch.logical_or(torch.isnan(depth_image), torch.isinf(depth_image))
                                )
                            ]
                            if len(valid_depth_vals) == 0:
                        ...
                        normalized_masked_depth_image = torch.clamp(
                            masked_depth_image / torch.max(masked_depth_image), 0.0, 1.0
                        )
                    ...
                    # forward through at_model / bs_model
                    if bs_model is not None and at_model is not None:
                        ...
                        if do_z_score:
                            ...
                            direct_filtered = torch.clamp(
                                (clamped_z * direct_std)
                                + torch.maximum(direct_mean, torch.Tensor([1.0 / 255]).cuda()),
                                0,
                                1,
                            )
                            direct = direct_filtered
                    ...
                    if viewpoint.original_depth_image is not None:
                        ...
                        gt_normalized_depth_image = torch.clamp(
                            gt_depth_image / torch.max(gt_depth_image), 0.0, 1.0
                        )

                    if at_criterion is not None and bs_model is not None and at_model is not None:
                    ...
                    if do_seathru and iteration > seathru_from_iter and bs_model is not None and at_model is not None:
                        reverse_direct = underwater_image - backscatter
                    ...
                    if tb_writer and (idx < 5):
                        ...
                        tb_writer.add_images(
                            f"{tag_header}/render", clamped_raw_render_image[None], global_step=iteration
                        )
                        tb_writer.add_images(
                            f"{tag_header}/depth_render", normalized_depth_image[None], global_step=iteration
                        )
                        tb_writer.add_images(
                            f"{tag_header}/alpha_image", image_alpha[None], global_step=iteration
                        )
                        if do_learn_bg:
                            tb_writer.add_images(
                                f"{tag_header}/bg_image", clamped_bg_image[None], global_step=iteration
                            )
                            tb_writer.add_images(
                                f"{tag_header}/render (with bg)", clamped_rgb_image[None], global_step=iteration
                            )
                            tb_writer.add_histogram(
                                f"{tag_header}/depth_histogram", depth_image, global_step=iteration
                            )
                            tb_writer.add_histogram(
                                f"{tag_header}/alpha_histogram", image_alpha, global_step=iteration
                            )
                        if bs_model is not None and at_model is not None:
                            tb_writer.add_images(
                                f"{tag_header}/backscatter_image",
                                torch.clamp(backscatter, 0.0, 1.0),
                                global_step=iteration,
                            )
                            tb_writer.add_images(
                                f"{tag_header}/backscatter_image_normalized",
                                torch.clamp(normalized_bs, 0.0, 1.0),
                                global_step=iteration,
                            )
                            tb_writer.add_images(
                                f"{tag_header}/attenuation_map",
                                torch.clamp(attenuation_map, 0.0, 1.0),
                                global_step=iteration,
                            )
                            tb_writer.add_images(
                                f"{tag_header}/attenuation_map_normalized",
                                torch.clamp(attenuation_map_normalized, 0.0, 1.0),
                                global_step=iteration,
                            )
                            tb_writer.add_images(
                                f"{tag_header}/attenuated_image",
                                direct,
                                global_step=iteration,
                            )
                            tb_writer.add_images(
                                f"{tag_header}/underwater_image",
                                underwater_image,
                                global_step=iteration,
                            )
                        if iteration == testing_iterations[0]:
                            tb_writer.add_images(
                                f"{tag_header}/ground_truth", gt_image[None], global_step=iteration
                            )
                            if viewpoint.original_depth_image is not None:
                                tb_writer.add_images(
                                    f"{tag_header}/gt_depth_image",
                                    gt_normalized_depth_image[None],
                                    global_step=iteration,
                                )

                    if bs_model is not None and at_model is not None:
                        l1_test += l1_loss_fn(underwater_image.squeeze(), gt_image).mean().double()
                    ...
                    if viewpoint.original_depth_image is not None:
                        depth_l1_test += l1_loss_fn(depth_image, gt_depth_image)
                ...
                if tb_writer:
                    ...
                    tb_writer.add_scalar(
                        config["name"] + "/loss_viewpoint - rgb_01_loss", rgb_01_test, iteration
                    )
                    tb_writer.add_scalar(config["name"] + "/loss_viewpoint - dcp_loss", dcp_test, iteration)
                    tb_writer.add_scalar(
                        config["name"] + "/loss_viewpoint - depth_smooth_loss",
                        depth_smooth_test,
                        iteration,
                    )
                    tb_writer.add_scalar(
                        config["name"] + "/loss_viewpoint - alpha_smooth_test",
                        alpha_smooth_test,
                        iteration,
                    )
                    ...
                    if viewpoint.original_depth_image is not None:
                        tb_writer.add_scalar(
                            config["name"] + "/loss_viewpoint - depth_l1_loss",
                            depth_l1_test,
                            iteration,
                        )
                    if bs_model is not None and at_model is not None:
                        tb_writer.add_scalar(
                            config["name"] + "/loss_viewpoint - rgb_sv_loss",
                            rgb_sv_test,
                            iteration,
                        )
                        tb_writer.add_scalar(
                            config["name"] + "/loss_viewpoint - rgb_sat_loss",
                            rgb_sat_test,
                            iteration,
                        )
                    if do_learn_bg:
                        tb_writer.add_scalar(
                            config["name"] + "/loss_viewpoint - alpha_bg_loss",
                            alpha_bg_test,
                            iteration,
                        )
                    if at_criterion is not None and bs_model is not None and at_model is not None:
                        tb_writer.add_scalar(
                            config["name"] + "/loss_viewpoint - dsc_at_loss",
                            dsc_at_test,
                            iteration,
                        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=int,
        default=[1_000, 7_000, 15_000, 30_000, 40_000, 50_000, 60_000],
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=int,
        default=[1_000, 7_000, 15_000, 30_000, 40_000],
    )
